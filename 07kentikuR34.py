...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if m_sum >= 0: #卒業＆進級要件と一致
                m_lst = ["学部専門系",'専門基礎科目群B科目',0,0,0]
                m_count = m_count + 1
                m_m_count = m_m_count + 1
                
            if n_sum >= 16: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',16,8,16]
                n_sum = n_sum - 16 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 8) & (n_sum < 16): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',16,8,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',16,8,n_sum]
            ...
            if mo_sum >= 42: #卒業＆進級要件と一致
                mo_lst = ["",'専門応用科目群B科目',42,42,42]
                mo_sum = mo_sum - 42 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                mo_lst =  ["",'専門応用科目群B科目',42,42,mo_sum]
                mo_sum = 0
    
            sentaku2_sum = n_sum + mo_sum + p_sum + q_sum + r_sum
            if sentaku2_sum >= 24: #卒業要件と一致
                sentaku2_lst = ["",'選択（専門）',24,16,24]
                sentaku2_sum = sentaku2_sum - 24 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (sentaku2_sum >= 16) & (sentaku2_sum < 24): #進級以上、卒業未満
                sentaku2_lst = ["",'選択（専門）',24,16,sentaku2_sum]
                sentaku2_sum = 0
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',24,16,sentaku2_sum]
...
root.title("令和3.4年度入学 工学部 建築学 ４年次進級＆卒業判定アプリ")
