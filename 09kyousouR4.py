...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            #学部専門系科目
            m2 = data[data['科目区分'] == '専門基礎科目群（必修）'] #協創のみ
            m2_sum = m2['単位数'].sum()
            ...
            #CSV破損検知
            all1_sum = a_sum + b_sum + b1_sum + c_sum + d_sum + e_sum + f_sum + g_sum + h_sum + i_sum + j_sum + k_sum + l_sum + m2_sum + m_sum + n_sum + o_sum + p_sum + q_sum + r_sum
            ...
            if sentaku_sum >= 12:
                sentaku_lst = ["",'選択',12,8,12]
                n_count = n_count + 1
                n_n_count = n_n_count + 1
                sentaku_sum = sentaku_sum -12
            elif (sentaku_sum >= 8) & (sentaku_sum < 12):
                sentaku_lst = ["",'選択',12,8,sentaku_sum]
                sentaku_sum = 0
                n_n_count = n_n_count + 1
            else:
                sentaku_lst = ["",'選択',12,8,sentaku_sum]
            ...
            if m2_sum >= 1: #卒業＆進級要件と一致
                m2_lst = ["学部専門系",'専門基礎科目群A科目',1,1,1]
                m2_sum = m2_sum - 1 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                m2_lst =  ["学部専門系",'専門基礎科目群A科目',1,1,m2_sum]
                m2_sum = 0

            if m_sum >= 9: #卒業＆進級要件と一致
                m_lst = ["",'専門基礎科目群B科目',9,9,9]
                m_sum = m_sum - 9 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                m_lst =  ["",'専門基礎科目群B科目',9,9,m_sum]
                m_sum = 0

            if n_sum >= 24: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',24,12,24]
                n_sum = n_sum - 24 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 12) & (n_sum < 24): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',24,12,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',24,12,n_sum]
            ...
            if mo_sum >= 36: #卒業要件と一致
                mo_lst = ["",'専門応用科目群B科目',36,34,36]
                mo_sum = mo_sum - 36 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (mo_sum >= 34) & (mo_sum < 36): #進級以上、卒業未満
                mo_lst = ["",'専門応用科目郡B科目',36,34,mo_sum]
                mo_sum = 0
                m_m_count = m_m_count + 1
            else:
                mo_lst =  ["",'専門応用科目群B科目',36,34,mo_sum]
                mo_sum = 0
    
            sentaku2_sum = m2_sum + n_sum + mo_sum + p_sum + q_sum + r_sum
            if sentaku2_sum >= 12: #卒業要件と一致
                sentaku2_lst = ["",'選択（専門）',12,10,12]
                sentaku2_sum = sentaku2_sum - 12 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (mo_sum >= 10) & (mo_sum < 12): #進級以上、卒業未満
                sentaku2_lst = ["",'選択（専門）',12,10,sentaku2_sum]
                sentaku2_sum = 0
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',12,10,sentaku2_sum]
            ...
            #表   
            table_data = [a_lst,b_lst,c_lst,d_lst,f_lst,i_lst,fgh_lst,ij_lst,sentaku_lst,m2_lst,m_lst,n_lst,mo_lst,sentaku2_lst,sonota_lst]
...
root.title("令和４年度入学 工学部 協創経営 ４年次進級＆卒業判定アプリ")
