...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if m_sum >= 6: #卒業＆進級要件と一致
                m_lst = ["学部専門系",'専門基礎科目群B科目',6,6,6]
                m_sum = m_sum - 6 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                m_lst =  ["学部専門系",'専門基礎科目群B科目',6,6,m_sum]
                m_sum = 0

            if n_sum >= 30: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',30,20,30]
                n_sum = n_sum - 30 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 20) & (n_sum < 30): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',30,20,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',30,20,n_sum]
            ...
            if mo_sum >= 38: #卒業要件と一致
                mo_lst = ["",'専門応用科目群B科目',38,32,38]
                mo_sum = mo_sum - 38 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (mo_sum >= 32) & (mo_sum < 38): #進級以上、卒業未満
                mo_lst = ["",'専門応用科目郡B科目',38,32,mo_sum]
                mo_sum = 0
                m_m_count = m_m_count + 1
            else:
                mo_lst =  ["",'専門応用科目群B科目',38,32,mo_sum]
                mo_sum = 0
    
            sentaku2_sum = n_sum + mo_sum + p_sum + q_sum + r_sum
            if sentaku2_sum >= 8: #卒業＆進級要件と一致
                sentaku2_lst = ["",'選択（専門）',8,8,8]
                sentaku2_sum = sentaku2_sum - 8 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',8,8,sentaku2_sum]
                sentaku2_sum = 0
...
root.title("令和3.4年度入学 工学部 人間支援感性科学 ４年次進級＆卒業判定アプリ")
