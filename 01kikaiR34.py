...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if m_sum >= 5: #卒業＆進級要件と一致
                m_lst = ["学部専門系",'専門基礎科目群B科目',5,5,5]
                m_sum = m_sum - 5 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                m_lst =  ["学部専門系",'専門基礎科目群B科目',5,5,m_sum]
                m_sum = 0

            if n_sum >= 39: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',39,31,39]
                n_sum = n_sum - 39 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 31) & (n_sum < 39): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',39,31,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',39,31,n_sum]
            ...
            if mo_sum >= 22: #卒業要件と一致
                mo_lst = ["",'専門応用科目群B科目',22,18,22]
                mo_sum = mo_sum - 22 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (mo_sum >= 18) & (mo_sum < 22): #進級以上、卒業未満
                mo_lst = ["",'専門応用科目郡B科目',22,18,mo_sum]
                mo_sum = 0
                m_m_count = m_m_count + 1
            else:
                mo_lst =  ["",'専門応用科目群B科目',22,18,mo_sum]
                mo_sum = 0
    
            sentaku2_sum = n_sum + mo_sum + p_sum + q_sum + r_sum
            if sentaku2_sum >= 16: #卒業要件と一致
                sentaku2_lst = ["",'選択（専門）',16,12,16]
                sentaku2_sum = sentaku2_sum - 16 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (sentaku2_sum >= 12) & (sentaku2_sum < 16): #進級以上、卒業未満
                sentaku2_lst = ["",'選択（専門）',16,12,sentaku2_sum]
                sentaku2_sum = 0
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',16,12,sentaku2_sum]
...
root.title("令和3.4年度入学 工学部 機械システム工学 ４年次進級＆卒業判定アプリ")
