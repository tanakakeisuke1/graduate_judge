...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if mo_sum >= 30:
                mo_lst = ["",'専門応用科目群B科目',30,26,30]
                mo_sum = mo_sum - 30
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (mo_sum >= 26) & (mo_sum < 30):
                mo_lst = ["",'専門応用科目郡B科目',30,26,mo_sum]
                mo_sum = 0
                m_m_count = m_m_count + 1
            else:
                mo_lst =  ["",'専門応用科目群B科目',30,26,mo_sum]
                mo_sum = 0
    
            sentaku2_sum = n_sum + mo_sum + p_sum + q_sum + r_sum
            if sentaku2_sum >= 8:
                sentaku2_lst = ["",'選択（専門）',8,8,8]
                sentaku2_sum = sentaku2_sum - 8
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
...
root.title("令和3.4年度入学 工学部 電子情報通信 ４年次進級＆卒業判定アプリ")
