...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if n_sum >= 31: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',31,21,31]
                n_sum = n_sum - 31 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 21) & (n_sum < 31): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',31,21,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',31,21,n_sum]
            ...
            if sentaku2_sum >= 7: #卒業＆進級要件と一致
                sentaku2_lst = ["",'選択（専門）',7,7,7]
                sentaku2_sum = sentaku2_sum - 7 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',7,7,sentaku2_sum]
                sentaku2_sum = 0
...
root.title("令和５年度入学 工学部 人間支援感性科学 ４年次進級＆卒業判定アプリ")
