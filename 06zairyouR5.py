...
def graduate_judge(file_path=None):
    ...
    try:
        ...
        if file_path:
            ...
            if n_sum >= 23: #卒業要件と一致
                n_lst = ["",'専門応用科目群A科目',23,15,23]
                n_sum = n_sum - 23 #卒業要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            elif (n_sum >= 15) & (n_sum < 23): #進級以上、卒業未満
                n_lst = ["",'専門応用科目群A科目',23,15,n_sum]
                n_sum = 0
                m_m_count = m_m_count + 1
            else:
                n_lst =  ["",'専門応用科目群A科目',23,15,n_sum]
            ...
            if sentaku2_sum >= 9: #卒業＆進級要件と一致
                sentaku2_lst = ["",'選択（専門）',9,9,9]
                sentaku2_sum = sentaku2_sum - 9 #卒業＆進級要件と一致
                m_count = m_count + 1
                m_m_count = m_m_count + 1
            else:
                sentaku2_lst = ["",'選択（専門）',9,9,sentaku2_sum]
                sentaku2_sum = 0
...
root.title("令和５年度入学 工学部 材料科学 ４年次進級＆卒業判定アプリ")
